...
class ArchiveActivity : AppCompatActivity() {
    private lateinit var archivesAdapter: ChatAdapter
    ...
    private fun initViews() {
        ...
        val touchCallback = ChatItemTouchHelperCallback(archivesAdapter) {item ->
            ...
            Snackbar.make(rv_archive_list, "Восстановить чат с ${item.title} из архива?", Snackbar.LENGTH_LONG)
                .setAction("Отмена") { viewModel.addToArchive(item.id) }
                .applyAppStyle()
                .show()
        }
    ...
    private fun initViewModel() {
        ...
        viewModel.getArchivesData().observe(this, Observer { archivesAdapter.updateData(it) })
    }
}
